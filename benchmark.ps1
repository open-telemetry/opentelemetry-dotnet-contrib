...
The BenchmarkDotNet project name to run. Accepts either the fully-qualified project name or a simplified project name.
...
.EXAMPLE
./benchmark.ps1 "Instrumentation.AspNetCore" @("*SamplerBenchmarks*") -SkipBaseline
...
foreach ($name in $candidateNames) {
    $candidate = Join-Path $testsPath "$name.Benchmarks" "$name.Benchmarks.csproj"
...
if (-not $project) {
    throw "Could not find a benchmark project for $ProjectName."
}
