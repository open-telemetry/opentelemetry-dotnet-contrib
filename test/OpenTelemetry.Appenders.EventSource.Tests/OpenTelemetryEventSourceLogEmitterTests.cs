...
public class OpenTelemetryEventSourceLogEmitterTests
{
    ...
    public void OpenTelemetryEventSourceLogEmitterSimpleEventTest()
    {
        ...
        Assert.Equal("OpenTelemetry.Appenders.EventSource", logRecord.CategoryName);
    ...
    public void OpenTelemetryEventSourceLogEmitterComplexEventTest(bool formatMessage)
    {
        ...
        if (formatMessage)
        {
            var formatString = TestEventSource.ComplexEventMessage;
            string expectedMessage = string.Format(CultureInfo.InvariantCulture, formatString, "Test_Message", 18);
    ...
    {
        private readonly List<System.Diagnostics.Tracing.EventSource> eventSources = [];
