    ...
    public class GenevaLogExporterTests
    {
        ...
        public void TableNameMappingTest(params string[] category)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // A stub server simulating the Geneva agent should be set up before construction of GenevaLogExporter
        ...
        private void AssertFluentdForwardModeForLogRecord(GenevaExporterOptions exporterOptions, object fluentdData, LogRecord logRecord)
        {
            ...
            */
            ...
            var partAVer = "4.0";
            var verKey = GenevaBaseExporter<LogRecord>.CS40_PART_A_MAPPING[Schema.V40.PartA.Ver];
            ...
            foreach (var item in exporterOptions.PrepopulatedFields)
            {
                ...
                var partAKey = GenevaBaseExporter<LogRecord>.CS40_PART_A_MAPPING[item.Key];
                Assert.Equal(partAValue, mapping[partAKey]);
            }

            var timeKey = GenevaBaseExporter<LogRecord>.CS40_PART_A_MAPPING[Schema.V40.PartA.Time];
