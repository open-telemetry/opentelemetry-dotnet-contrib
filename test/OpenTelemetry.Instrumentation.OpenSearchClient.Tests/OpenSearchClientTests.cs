...
public class OpenSearchClientTests
{
    ...
    public async Task CanCaptureGetById()
    {
        ...
        var client = new OpenSearch.Client.OpenSearchClient(new ConnectionSettings(new InMemoryConnection()).DefaultIndex("customer"));
        ...
        Assert.Equal($"OpenSearch GET customer", searchActivity.DisplayName);
    ...
    public async Task CanCaptureGetByIdNotFound()
    {
        ...
        var client = new OpenSearch.Client.OpenSearchClient(new ConnectionSettings(new InMemoryConnection(null, statusCode: 404)).DefaultIndex("customer"));
    ...
    public async Task CanCaptureSearchCall()
    {
        ...
        Assert.Equal($"OpenSearch POST customer", searchActivity.DisplayName);
    ...
    public async Task CanRecordAndSampleSearchCall()
    {
        ...
        var client = new OpenSearch.Client.OpenSearchClient(new ConnectionSettings(new InMemoryConnectionWithDownstreamActivity()).DefaultIndex("customer").EnableDebugMode());

        using (Sdk.CreateTracerProviderBuilder()
                   .SetSampler(sampler)
                   .AddSource("Downstream")
                   .AddSource("NestedDownstream")
                   .AddOpenSearchClientInstrumentation((opt) => opt.SuppressDownstreamInstrumentation = false)
    ...
    public async Task CanSuppressDownstreamActivities()
    {
        ...
        using (Sdk.CreateTracerProviderBuilder()
                   ...
                   .AddSource("Downstream")
                   .AddSource("NestedDownstream")
                   .AddOpenSearchClientInstrumentation((opt) => opt.SuppressDownstreamInstrumentation = true)
                   .AddProcessor(testActivityProcessor)
                   .Build())
        {
            var searchResponse = await client.SearchAsync<Customer>(s => s.Query(q => q.Bool(b => b.Must(m => m.Term(f => f.Id, "123")))));
    ...
    public async Task CanCaptureSearchCallWithDebugMode()
    {
        ...
        var client = new OpenSearch.Client.OpenSearchClient(new ConnectionSettings(new InMemoryConnection()).DefaultIndex("customer").EnableDebugMode());
    ...
    public async Task CanCaptureSearchCallWithParseAndFormatRequestOption()
    {
        ...
        using (Sdk.CreateTracerProviderBuilder()
                   .SetSampler(new AlwaysOnSampler())
                   .AddOpenSearchClientInstrumentation(o => o.ParseAndFormatRequest = true)
                   .SetResourceBuilder(expectedResource)
                   .AddInMemoryExporter(exportedItems)
                   .Build())
        {
  ...
  ""query"": {
    ...
    public async Task CanCaptureMultipleIndiceSearchCall()
    {
        ...
        var client = new OpenSearch.Client.OpenSearchClient(new ConnectionSettings(new InMemoryConnection()).DefaultIndex("customer,order"));
        ...
        Assert.Equal($"OpenSearch POST", searchActivity.DisplayName);
    ...
    [Fact]
    public async Task CanCaptureOpenSearchClientException()
    {
        var expectedResource = ResourceBuilder.CreateDefault().AddService("test-service");
        var exportedItems = new List<Activity>();
        ...
        var client = new OpenSearch.Client.OpenSearchClient(new ConnectionSettings(connection).DefaultIndex("customer").EnableDebugMode());
    ...
    public async Task CanCaptureCatRequest()
    {
        ...
        Assert.Equal($"OpenSearch GET", searchActivity.DisplayName);
    ...
    public async Task DbStatementIsNotDisplayedWhenSetDbStatementForRequestIsFalse()
    {
        ...
        using (Sdk.CreateTracerProviderBuilder()
                   .SetSampler(new AlwaysOnSampler())
                   .AddOpenSearchClientInstrumentation(o => o.SetDbStatementForRequest = false)
                   .SetResourceBuilder(expectedResource)
                   .AddInMemoryExporter(exportedItems)
                   .Build())
        {
    ...
    public async Task ShouldRemoveSensitiveInformation()
    {
        ...
        var client = new OpenSearch.Client.OpenSearchClient(new ConnectionSettings(
