...
public class ContainerResourceDetectorTests
{
    ...
    {
        var dockerResourceDetector = new ContainerResourceDetector();
    ...
    public void TestInvalidContainer()
    {
        ...
        foreach (var testCase in this.testValidCasesV1)
        {
            ...
            tempFile.Write(testCase.Line);
            Assert.Equal(
                dockerResourceDetector.BuildResource(tempFile.FilePath, ContainerResourceDetector.ParseMode.V2),
                Resource.Empty);
        }
        ...
        foreach (var testCase in this.testValidCasesV2)
        {
            ...
            tempFile.Write(testCase.Line);
            Assert.Equal(
                dockerResourceDetector.BuildResource(tempFile.FilePath, ContainerResourceDetector.ParseMode.V1),
                Resource.Empty);
        }
        ...
        // test invalid file
        Assert.Equal(dockerResourceDetector.BuildResource(Path.GetTempPath(), ContainerResourceDetector.ParseMode.V1), Resource.Empty);
        Assert.Equal(dockerResourceDetector.BuildResource(Path.GetTempPath(), ContainerResourceDetector.ParseMode.V2), Resource.Empty);
    }
    ...
    private class TestCase
    {
        ...
        public ContainerResourceDetector.ParseMode CgroupVersion { get; set; }
    }
}
