...
#if NET
        typeof(Activity).GetProperty("Source")!
            .SetMethod!.CreateDelegate<Action<Activity, ActivitySource>>();
#else
        (Action<Activity, ActivitySource>)typeof(Activity).GetProperty("Source")
...
#if NET
        typeof(Activity).GetProperty("Kind")!
            .SetMethod!.CreateDelegate<Action<Activity, ActivityKind>>();
#else
        (Action<Activity, ActivityKind>)typeof(Activity).GetProperty("Kind")
