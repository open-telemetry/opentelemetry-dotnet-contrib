...
The client retains up to 2,048 custom messages while another message is being
sent. `SendCustomMessage()` throws `InvalidOperationException` when that queue
is full. Await `FlushAsync()` before retrying the rejected message.
