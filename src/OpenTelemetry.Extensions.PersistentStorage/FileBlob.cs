...
namespace OpenTelemetry.Extensions.PersistentStorage
{
    ...
    public class FileBlob : PersistentBlob
    {
        ...
        protected override bool OnTryRead([NotNullWhen(true)] out byte[] buffer)
        {
            ...
            {
                PersistentStorageEventSource.Log.PersistentStorageException(nameof(FileBlob), $"Reading a blob from file {this.FullPath} has failed.", ex);
                buffer = null;
                return false;
            }
        ...
        protected override bool OnTryWrite(byte[] buffer, int leasePeriodMilliseconds = 0)
        {
            ...
            {
                PersistentStorageEventSource.Log.PersistentStorageException(nameof(FileBlob), $"Writing a blob to file {path} has failed.", ex);
                return false;
            }
        ...
        protected override bool OnTryLease(int leasePeriodMilliseconds)
        {
            ...
            {
                PersistentStorageEventSource.Log.PersistentStorageException(nameof(FileBlob), $"Acquiring a lease to file {this.FullPath} has failed.", ex);
                return false;
            }
        ...
        protected override bool OnTryDelete()
        {
            ...
            {
                PersistentStorageEventSource.Log.PersistentStorageException(nameof(FileBlob), $"Deletion of file blob {this.FullPath} has failed.", ex);
                return false;
            }
