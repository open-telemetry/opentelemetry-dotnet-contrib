...
namespace OpenTelemetry.Instrumentation.OpenSearchClient.Implementation;

internal class OpenSearchRequestPipelineDiagnosticListener : ListenerHandler
{
    ...
    internal static readonly Assembly Assembly = typeof(OpenSearchRequestPipelineDiagnosticListener).Assembly;
    ...
    private readonly OpenSearchClientInstrumentationOptions options;
    ...
    public OpenSearchRequestPipelineDiagnosticListener(OpenSearchClientInstrumentationOptions options)
    ...
    private static string GetDisplayName(Activity activity, object? method, string? indexName = null)
    {
        switch (activity.OperationName)
        {
            ...
            case "CallOpenSearch" when method != null:
                {
                    var methodName = MethodNameCache.GetOrAdd(method, $"OpenSearch {method}");
                    return indexName == null ? methodName : $"{methodName} {indexName}";
                }
    ...
    private static string? GetIndexName(Uri uri)
    {
        ...
        var indexName = Uri.UnescapeDataString(uri.Segments[1]);
        ...
        if (indexName.EndsWith("/", StringComparison.Ordinal))
        {
    ...
    private void OnStartActivity(Activity activity, object? payload)
    {
        ...
        if (activity.IsAllDataRequested)
        {
            ...
            if (uri == null)
            {
                OpenSearchInstrumentationEventSource.Log.NullPayload(nameof(OpenSearchRequestPipelineDiagnosticListener), nameof(this.OnStartActivity));
                return;
            }
            ...
            var indexName = GetIndexName(uri);
            activity.DisplayName = GetDisplayName(activity, method, indexName);
            activity.SetTag(SemanticConventions.AttributeDbSystem, DatabaseSystemName);

            if (indexName != null)
            {
                activity.SetTag(SemanticConventions.AttributeDbName, indexName);
            }
            ...
            {
                OpenSearchInstrumentationEventSource.Log.EnrichmentException(ex);
            }
        }
    }
