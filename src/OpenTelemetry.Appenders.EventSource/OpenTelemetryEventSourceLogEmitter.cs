...
{
    private static readonly Tuple<LogRecordSeverity, string>[] EventLevels =
    [
        ...
        new(LogRecordSeverity.Warn, nameof(EventLevel.Warning)),
        new(LogRecordSeverity.Info, nameof(EventLevel.Informational)),
    ...
    private readonly List<EventSource> eventSources = [];
        ...
        bool disposeProvider = false)
    {
        ...
        this.logger = loggerProvider.GetLogger("OpenTelemetry.Appenders.EventSource", $"semver:{typeof(OpenTelemetryEventSourceLogEmitter).Assembly.GetName().Version}");
