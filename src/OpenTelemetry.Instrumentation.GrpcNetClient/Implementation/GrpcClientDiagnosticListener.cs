...
internal sealed class GrpcClientDiagnosticListener : ListenerHandler
{
    ...
    public void OnStartActivity(Activity activity, object? payload)
    {
        ...
        // A filtered-out request clears the Recorded flag, so the filter has to
        // run before the context is injected below.
        if (activity.IsAllDataRequested && !this.options.EventFilter(request))
        {
        ...
        // HttpClient only adds trace context headers that are missing, so
        // injecting here parents the downstream span to the Grpc.Net.Client
        // span unless HttpClient instrumentation injects too.
