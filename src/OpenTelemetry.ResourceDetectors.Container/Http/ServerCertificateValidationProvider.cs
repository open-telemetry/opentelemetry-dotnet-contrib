...
internal class ServerCertificateValidationProvider
{
    ...
    public static ServerCertificateValidationProvider? FromCertificateFile(string certificateFile)
    {
        ...
        {
            ContainerExtensionsEventSource.Log.FailedToValidateCertificate(nameof(ServerCertificateValidationProvider), "Certificate File does not exist");
            return null;
        }
        ...
        {
            ContainerExtensionsEventSource.Log.FailedToValidateCertificate(nameof(ServerCertificateValidationProvider), "Failed to load certificate in trusted collection");
            return null;
        }
    ...
    private bool ValidateCertificate(X509Certificate2? cert, X509Chain? chain, SslPolicyErrors errors)
    {
        ...
        if (!isSslPolicyPassed)
        {
            ...
            {
                ContainerExtensionsEventSource.Log.FailedToValidateCertificate(nameof(ServerCertificateValidationProvider), "Failed to validate certificate due to RemoteCertificateNotAvailable");
            }

            if ((errors | SslPolicyErrors.RemoteCertificateNameMismatch) == errors)
            {
                ContainerExtensionsEventSource.Log.FailedToValidateCertificate(nameof(ServerCertificateValidationProvider), "Failed to validate certificate due to RemoteCertificateNameMismatch");
            }
        }

        if (chain == null)
        {
            ContainerExtensionsEventSource.Log.FailedToValidateCertificate(nameof(ServerCertificateValidationProvider), "Failed to validate certificate. Certificate chain is null.");
            return false;
        }

        if (cert == null)
        {
            ContainerExtensionsEventSource.Log.FailedToValidateCertificate(nameof(ServerCertificateValidationProvider), "Failed to validate certificate. Certificate is null.");
            return false;
        }
        ...
        if (!isValidChain)
        {
            ...
            ContainerExtensionsEventSource.Log.FailedToValidateCertificate(nameof(ServerCertificateValidationProvider), $"Failed to validate certificate due to {chainErrors}");
        }
        ...
        if (!isTrusted)
        {
            ...
            ContainerExtensionsEventSource.Log.FailedToValidateCertificate(
