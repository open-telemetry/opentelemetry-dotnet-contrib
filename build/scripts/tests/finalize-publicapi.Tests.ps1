...
Describe "finalize-publicapi.ps1" {
    ...
    # Note: the merge uses a '.publicApi\**\PublicAPI.Shipped.txt' glob that only
    # resolves correctly on Windows, so the merge assertion is Windows-only. The
    # non-matching case below exercises the script on every platform.
    It "merges, sorts and de-duplicates unshipped public APIs into shipped" -Skip:(-not $IsWindows) {
        ...
        try {
            & $scriptPath -minVerTagPrefix "foo-" 6>$null
        }
        finally {
            Pop-Location
        }
    ...
    It "does not touch projects that do not match the tag prefix" {
        ...
        $project = Join-Path -Path $work -ChildPath "src/OpenTelemetry.Instrumentation.Other"
        $apiDirectory = Join-Path -Path $project -ChildPath ".publicApi/Stable"

        New-Item -Path $apiDirectory -ItemType Directory -Force | Out-Null

        Set-Content `
            -Path (Join-Path -Path $project -ChildPath "OpenTelemetry.Instrumentation.Other.csproj") `
